...
int main()
{
    ...
    // Declare a new variable to track if the player is dead
    ...
    // Add semi-transparent rectangles for HUD
    RectangleShape scoreBackground(Vector2f(550, 100)); // Increased size
    scoreBackground.setFillColor(Color(0, 0, 0, 150));
    scoreBackground.setPosition(15, 30);

    RectangleShape fpsBackground(Vector2f(240, 55)); // Increased size
    fpsBackground.setFillColor(Color(0, 0, 0, 150));
    fpsBackground.setPosition(15, 163);

    // Optimize cloud code using arrays
    const int NUM_CLOUDS = 6;
    Sprite clouds[NUM_CLOUDS];
    bool cloudActive[NUM_CLOUDS] = { false };
    float cloudSpeed[NUM_CLOUDS] = { 0.0f };

    // Initialize clouds
    for (int i = 0; i < NUM_CLOUDS; i++) {
        clouds[i].setTexture(textureCloud);
        clouds[i].setPosition(-200, i * 150);
    }
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            // To Pause/Unpause the Game or Restart after death on Pressing 'ENTER'
            if (event.type == Event::KeyPressed && event.key.code == Keyboard::Enter)
            {
                ...
                {
                    // Pause or unpause the game if the player is not dead
                    Pause = !Pause;
                    if (!Pause)
                    {
                        clock.restart();
        ...
        if (!Pause)
        {
            ...
            if (branchPositions[5] == playerSide && !playerDead)
            {
                // Death
                Pause = true;
                playerDead = true;
            ...
            // Update the score text occasionally
            if (frameCount % 10 == 0) {
        ...
        // Step 17: Draw 3 Cloud sprites (moved above branches and trees)
        for (int i = 0; i < NUM_CLOUDS; i++) {
            window.draw(clouds[i]);
        }

        // Step 16: Draw the bee sprite (moved to be above clouds)
